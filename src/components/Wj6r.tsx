...
export const SystemDashboard: React.FC<Props> = ({ integration }) => {
  ...
  useEffect(() => {
    const subs = [
      integration.autonomic.getHyperfocusMetrics().subscribe(hyperfocus => {
        setMetrics(m => ({ ...m, hyperfocus }));
      }),
      integration.autonomic.getEnergyMetrics().subscribe(energy => {
        setMetrics(m => ({ ...m, energy }));
      }),
      integration.flow.observeFlowState().subscribe(flow => {
        setMetrics(m => ({ 
          ...m, 
          flow: {

...
export interface PatternLibrary {
  ...
  getMostSuccessfulPatterns: (limit?: number) => Pattern[];
  getPatternsByEnergy: (energy: number, tolerance?: number) => Pattern[];
  getPatternsByFlow: (flow: string) => Pattern[];
  getPatternsByContext: (context: string[]) => Pattern[];
  getPatternsByState: (states: string[]) => Pattern[];
  subscribe: (callback: (patterns: Pattern[]) => void) => { unsubscribe: () => void };
}
...
export interface PatternManager {
  ...
  subscribe: (callback: (state: { current: Pattern | null; history: Pattern[] }) => void) => { unsubscribe: () => void };
}

...
const SpaceCard = styled(MetricCard)`
  position: relative;
  overflow: hidden;
...
export const SystemDashboard: React.FC<Props> = ({ integration }) => {
  ...
  const calculateHarmony = (metrics: { [key: string]: number }): number => {
    return Object.values(metrics).reduce((sum, val) => sum + val, 0) / Object.values(metrics).length;
  };

  return (
    <DashboardContainer>
      <MetricCard harmony={calculateHarmony(state.presence)}>
        <MetricHeader>Divine Presence</MetricHeader>
      ...
      <MetricCard harmony={calculateHarmony(state.energy)}>
        <MetricHeader>Living Energy</MetricHeader>
      ...
      >
        <MetricHeader>Sacred Space</MetricHeader>
      ...
      <MetricCard harmony={calculateHarmony(state.transition)}>
        <MetricHeader>Space Transition</MetricHeader>

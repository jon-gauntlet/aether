...
describe('Attention Metrics', () => {
  ...
  describe('attention calculation', () => {
    it('calculates metrics from system state', () => {
      ...
      expect(metrics.focusDepth).toBeCloseTo(0.8, 2)
      expect(metrics.contextClarity).toBeCloseTo(0.7, 2)
      expect(metrics.energyReserves).toBeCloseTo(0.9, 2)
      expect(metrics.momentum).toBeCloseTo(0.6, 2) // recentProgress - blockers
    })
    ...
    it('handles missing context values', () => {
      ...
      expect(metrics.contextClarity).toBeCloseTo(1, 2) // Default
      expect(metrics.momentum).toBeCloseTo(0, 2) // No progress, no blockers
    })
  })

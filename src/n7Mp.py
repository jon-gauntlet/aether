...
def test_empty_array():
    ...
    assert_equal(-1, result, "Searching empty array returns -1")

def test_single_element():
    """Understanding base cases"""
    single = [42]
    result = binary_search(single, 42)
    assert_equal(0, result, "Found element should return its index")
    
    result = binary_search(single, 43)
    assert_equal(-1, result, "Missing element should return -1")

def test_multiple_elements():
    """Understanding binary search"""
    numbers = [1, 3, 5, 7, 9, 11, 13]
    result = binary_search(numbers, 7)
    assert_equal(3, result, "Should find element in middle")
    
    result = binary_search(numbers, 13)
    assert_equal(6, result, "Should find element at end")
    
    result = binary_search(numbers, 1)
    assert_equal(0, result, "Should find element at start")

def test_missing_element():
    """Understanding not found cases"""
    numbers = [1, 3, 5, 7, 9]
    result = binary_search(numbers, 4)
    assert_equal(-1, result, "Should return -1 for missing element")
    
    result = binary_search(numbers, 0)
    assert_equal(-1, result, "Should return -1 for element below range")
    
    result = binary_search(numbers, 10)
    assert_equal(-1, result, "Should return -1 for element above range")

            ...
            for c_marker in self.consensus_markers:
                if c_marker.lower() in para.lower():
                    ...
                    if contrarian_view:
                        ...
                        consensus = self._extract_consensus(para)
                        contrarian = self._extract_contrarian(contrarian_view)
                        
                        # Only add if both views are meaningful
                        if len(consensus) > 20 and len(contrarian) > 20:
                            pov = SpikyPOV(
                                title=f"POV from: {article.title}",
                                consensus_view=self._clean_text(consensus),
                                contrarian_view=self._clean_text(contrarian),
                                evidence=evidence,

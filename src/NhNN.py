...
def save_findings_to_json(findings, output_file):
    with open(output_file, 'w') as file:
        json.dump(findings, file, indent=4)

# Example usage function for testing
def run_parsing(input_directory, output_file):
    findings = parse_all_markdown_files(input_directory)
    save_findings_to_json(findings, output_file)
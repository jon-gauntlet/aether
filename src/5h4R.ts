...
export class ConsciousnessEngine {
  ...
  private connectThoughtToSpace(thoughtId: string, spaceId: string) {
    ...
    if (thought && space) {
      ...
      this.thoughts.get(thoughtId)?.next(updatedThought);
      ...
      const updatedSpace = {
        ...
        state: {
          ...space.state,
          resonance: this.harmonizeResonances(
            space.state.resonance,
            thought.resonance
          )
        }
      };
      this.spaces.get(spaceId)?.next(updatedSpace);
    }
  }
  ...
  private harmonizeResonances(a: Resonance, b: Resonance, c?: Resonance): Resonance {
    const resonances = c ? [a, b, c] : [a, b];
    
    const frequency = resonances.reduce((sum, r) => sum + r.frequency, 0) / resonances.length;
    const amplitude = resonances.reduce((sum, r) => sum + r.amplitude, 0) / resonances.length;
    const harmony = resonances.reduce((sum, r) => sum + r.harmony, 0) / resonances.length;
  ...
  private findConnectedThoughts(spaceId: string): ThoughtStream[] {
    const connectedThoughts: ThoughtStream[] = [];
    
    this.thoughts.forEach((thoughtSubject) => {
      const thought = thoughtSubject.value;
      if (thought.connections.some(conn => conn.targetId === spaceId)) {
        connectedThoughts.push(thought);
      }
    });

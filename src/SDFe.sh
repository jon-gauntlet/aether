...
mkdir -p ~/.local/state/cursor/{autonomic,backups,processed,meta,harmony,focus,patterns,synthesis}
...
chmod +x ~/scripts/cursor/pattern-synthesizer
...
cp ~/scripts/cursor/pattern-synthesizer.service ~/.config/systemd/user/
...
# Initialize pattern synthesis state
if [[ ! -f ~/.local/state/cursor/synthesis/state.json ]]; then
    echo '{
        "version": 1,
        "last_update": 0,
        "patterns": [],
    ...
    }' > ~/.local/state/cursor/synthesis/state.json
fi
...
systemctl --user enable --now pattern-synthesizer.service
systemctl --user enable --now context-crystallizer.service
systemctl --user enable --now essence-integrator.service

echo "Context preservation, autonomic management, meta-learning, harmony, focus, and pattern synthesis services installed and started"
...
systemctl --user status pattern-synthesizer.service
...
ln -sf ~/scripts/cursor/pattern-synthesizer ~/bin/cursor-patterns
...
alias cpatterns='cursor-patterns status'
EOF
...
echo "  cpatterns - Check pattern synthesis" 
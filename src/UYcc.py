...
def parse_markdown_file(file_path):
    ...
    # Extract the title, removing any leading number, dot, and space
    ...
    # Extract the current code, including the code block delimiters
    current_code_match = re.search(r'Current Code:\n(```[\s\S]*?```)', content)
    current_code = current_code_match.group(1).strip() if current_code_match else ''

    # Extract the secure code, including the code block delimiters

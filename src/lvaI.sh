...
# Strict check for sourcing
if [[ $ZSH_EVAL_CONTEXT == 'toplevel:file' || $ZSH_EVAL_CONTEXT == 'file' || $0 != "$ZSH_ARGZERO" ]]; then
    return 0
fi
...
# Exit if no tty (being sourced or run in non-interactive mode)

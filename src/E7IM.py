...
def test_function_basics():
    ...
    result = greet("World")
    assert_equal(__, result, "Functions can take parameters and return values")
...
def test_multiple_parameters():
    ...
    result = multiply(4, 5)
    assert_equal(__, result, "Functions can take multiple parameters")
    
    result2 = multiply(3.0, 2)
    assert_equal(__, type(result2), "Type of result depends on input types")
...
def test_default_arguments():
    ...
    square = power(4)
    assert_equal(__, square, "Default argument is used when not specified")
    
    cube = power(3, 3)
    assert_equal(__, cube, "Default argument can be overridden")
...
def test_return_multiple_values():
    ...
    assert_equal(__, quotient, "First returned value")
    assert_equal(__, remainder, "Second returned value")

def test_lambda_functions():
    """Understanding lambda functions"""
    square = lambda x: x * x
    assert_equal(__, square(4), "Lambda functions are concise")
    
    numbers = [1, 2, 3, 4, 5]
    evens = list(filter(lambda x: x % 2 == 0, numbers))
    assert_equal(__, evens, "Lambda functions work with built-ins")

...
class ModelService:
    ...
    async def _queue_initial_downloads(self):
        """Queue downloads for essential models"""
        essential_models = ['llama2-7b-q4', 'codellama-7b-q4']  # Start with smaller models
        for model in essential_models:
            if model not in self.downloader.get_available_models():
                await self.request_download(model)
    ...
    async def _process_downloads(self):
        ...
        while self.active:
            try:
                ...
                # Skip if another download of same model is in progress
                ...
                # Queue next model if this was successful
                if success and self.active:
                    await self._queue_next_model(model_name)
    ...
    async def _queue_next_model(self, completed_model: str):
        """Queue next logical model based on dependencies"""
        model_dependencies = {
            'llama2-7b-q4': ['llama2-13b-q4'],
            'codellama-7b-q4': ['phi-2-q4']
        }
        
        if completed_model in model_dependencies:
            for next_model in model_dependencies[completed_model]:
                if next_model not in self.downloader.get_available_models():
                    await self.request_download(next_model)

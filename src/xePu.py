...
class ZAPParser(BaseParser):
            ...
            for alert in site.get('alerts', []):
                try:
                    ...
                    normalized['metadata'] = {
                        ...
                        'cwe_ids': [str(alert.get('cweid'))] if alert.get('cweid') and alert['cweid'] != '-1' else [],
    ...
    def _map_confidence(self, confidence: str) -> str:
        """Map ZAP confidence level to normalized confidence.
        
        Args:
            confidence: ZAP confidence level (0-3)
    ...
    def _get_reproduction_steps(self, alert: Dict[str, Any], site: Dict[str, Any]) -> List[str]:
        ...
        steps.extend([
            ...
            f"Confidence: {self._map_confidence(alert.get('confidence', '0'))}"
        ])
    ...
    def _generate_tags(self, alert: Dict[str, Any]) -> List[str]:
        ...
        tags = [
            ...
            f"confidence:{self._map_confidence(alert.get('confidence', '0')).lower()}",

...
class RiskScorer:
    ...
    def adjust_severity_for_context(self, finding: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            base_severity = str(finding.get('severity', 'LOW')).upper()
            if base_severity not in ['LOW', 'MEDIUM', 'HIGH', 'CRITICAL', 'INFO']:
                base_severity = 'LOW'
            ...
            try:
                ...
                current_idx = 0  # Default to LOW if severity not in standard levels
            ...
            if any(term in str(finding.get('description', '')).upper() for term in 
            ...
            if any(req.lower() in str(finding.get('description', '')).lower() 
            ...
            logger.warning(f"Error processing finding severity: {e}")
            finding['severity'] = 'LOW'  # Default to LOW on error
            finding['severity_adjustment_reason'] = "Defaulted due to processing error"
            return finding
    ...
    def calculate_risk_score(self, finding: Dict[str, Any]) -> float:
        ...
        try:
            ...
            base_severity = str(finding.get('severity', 'LOW')).upper()
            base_score = severity_map.get(base_severity, 3.0)
            ...
            return 3.0  # Default to medium-low risk on error

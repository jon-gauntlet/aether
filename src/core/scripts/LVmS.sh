...
# Create the directory for environment files
mkdir -p /etc/gauntlet
chown root:root /etc/gauntlet
chmod 755 /etc/gauntlet
...
chown root:$ORIGINAL_USER /etc/gauntlet/*.env
chmod 640 /etc/gauntlet/*.env
...
After=network.target gauntlet-optimizer.service
Wants=gauntlet-optimizer.service
ConditionPathExists=/etc/gauntlet/metrics.env
...
After=network.target system-ai.slice
Wants=system-ai.slice
ConditionPathExists=/etc/gauntlet/optimizer.env
StartLimitIntervalSec=300
StartLimitBurst=3
...
$ORIGINAL_USER ALL=(ALL) NOPASSWD: /usr/bin/tee /sys/devices/system/cpu/cpu[0-9]*/cpufreq/scaling_governor
$ORIGINAL_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart gauntlet-*.service
$ORIGINAL_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart chatgenius-dev.service
$ORIGINAL_USER ALL=(ALL) NOPASSWD: /usr/bin/journalctl
Defaults!GAUNTLET_CMDS env_keep += "GAUNTLET_HOME CURSOR_WORKSPACE CURSOR_PROJECT"
EOF

chmod 440 /etc/sudoers.d/99-gauntlet
chown root:root /etc/sudoers.d/99-gauntlet

...
export async function generateAIFeedback({
  ...
  if (cachedFeedback) {
    try {
      return JSON.parse(cachedFeedback as string) as FeedbackResponse
    } catch (error) {
      console.error('Failed to parse cached feedback:', error)
...
export async function analyzeCodeSubmission(
  ...
  if (!completion.choices[0].message.content) {
    throw new Error('Failed to analyze code submission')
  }

...
import { EnergyState, NaturalFlowType } from '../types/base';
...
const energyState = energySystem.createState();

export const useAutonomicDevelopment = () => {
  const [currentMode, setCurrentMode] = useState<NaturalFlowType>('natural');

  const setMode = (mode: NaturalFlowType) => {
    setCurrentMode(mode);
    flowGuardian.setMode(flowContext, mode);
  };
  ...
  useEffect(() => {
    ...
    const subscription = combineLatest([
      ...
      new BehaviorSubject<EnergyState>(energyState),

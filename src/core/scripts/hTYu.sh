    ...
    for service in context-crystallizer essence-integrator; do
        ...
        if [[ -s "$PATTERN_DB" ]]; then
            ...
            if [[ -n "$service_patterns" ]] && [[ "$service_patterns" != "null" ]]; then
                local current_pressure=$(echo "$metrics" | jq -r --arg svc "$service" '.[$svc].pressure')
                local pattern_pressure=$(echo "$service_patterns" | jq -r --arg svc "$service" '[.[] | select(.service == $svc) | .metrics.pressure] | if length > 0 then max else "0" end')
                
                if (( $(echo "$current_pressure > $pattern_pressure" | bc -l) )); then
        ...
        if [[ "$status" =~ "NRestarts" ]]; then
            local restarts=$(echo "$status" | grep -oP 'NRestarts=\K\d+')
            if (( restarts > 3 )); then

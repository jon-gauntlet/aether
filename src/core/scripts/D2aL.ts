...
export class PredictiveValidation {
  ...
  private estimateSuccessProbability(
    ...
    const energyFactor = (energy.current / energy.max) * 0.3;
    const focusFactor = energy.focusMultiplier * 0.2;
    const successFactor = (pattern.indicators.validation_success.slice(-1)[0] || 0.5) * 0.3;
    const confidenceFactor = autonomic.confidence * 0.2;

    return Math.min(1, energyFactor + focusFactor + successFactor + confidenceFactor);
  }

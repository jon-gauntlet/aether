...
create_json_metrics() {
    ...
    jq -n \
        ...
        --argjson deep "$(get_json_value "$focus_metrics_file" "deep_mode_duration" "0")" \
        --argjson flow "$(get_json_value "$focus_metrics_file" "flow_state_duration" "0")" \
        --argjson focus "$(get_json_value "$focus_metrics_file" "focus_sessions" "0")" \
        '{
            timestamp: $ts,
            daily_stats: $stats,
            system_metrics: {
...
optimize_gauntlet() {
    ...
    # Create metrics JSON
    if ! create_json_metrics "$cpu_usage" "$mem_usage" "$swap_usage" "$disk_usage" "$current_stats" "$focus_metrics_file" > "$temp_file"; then

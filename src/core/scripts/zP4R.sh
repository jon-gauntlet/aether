...
optimize_gauntlet() {
    ...
    local cpu_usage=$(top -bn1 | grep "Cpu(s)" | awk '{print $2}' || echo "0")
    local mem_total=$(free | grep Mem | awk '{print $2}')
    local mem_used=$(free | grep Mem | awk '{print $3}')
    local swap_total=$(free | grep Swap | awk '{print $2}')
    local swap_used=$(free | grep Swap | awk '{print $3}')
    local disk_usage=$(df -h / | tail -1 | awk '{print $5}' | tr -d '%' || echo "0")
    
    local mem_usage=$(calc_percentage "$mem_used" "$mem_total")
    local swap_usage=$(calc_percentage "$swap_used" "$swap_total")

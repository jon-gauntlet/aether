...
export function balance(rooms: Room[]) {
  ...
  rooms.forEach(room => {
    ...
    if (effects.length > 0) {
      room.calm = blend(room.calm, average(effects.map((e: Effect) => e.calm)));
      room.focus = blend(room.focus, average(effects.map((e: Effect) => e.focus)));
...
function findConnected(room: Room, rooms: Room[]) {
  return room.paths
    ...
    .filter((n: { room: Room | undefined }) => n.room);
}

...
preflight_check() {
    ...
    # Remove unused cached packages while keeping the latest version
    echo "🧹 Cleaning package cache..."
    paccache -rk1
    
    # Keep essential messaging apps
    KEEP_PKGS="telegram-desktop signal-desktop element-desktop"
    
    # Get list of installed GUI apps excluding essential ones
    GUI_APPS=$(pacman -Qe | grep -E 'game|media|office|graphics' | grep -vE "$(echo $KEEP_PKGS | tr ' ' '|')" || true)
    
    if [ ! -z "$GUI_APPS" ]; then
        echo "🧹 Removing non-essential GUI applications..."
        pacman -Rns --noconfirm $GUI_APPS
    fi
    
    # Check for conflicting Python installations
    echo "🐍 Checking Python environment..."
    if pacman -Q python2 &>/dev/null; then
        echo "Removing Python 2..."
        pacman -Rns --noconfirm python2
    fi
    
    # Remove potentially conflicting AUR helpers
    for helper in yay paru pamac; do
        if pacman -Q $helper &>/dev/null; then
            echo "Removing $helper..."
            pacman -Rns --noconfirm $helper
        fi
    done
    
    # Clean up old Python virtual environments
    echo "🧹 Cleaning up old virtual environments..."
    find $USER_HOME -type d -name ".venv" -exec rm -rf {} + 2>/dev/null || true
    find $USER_HOME -type d -name "venv" -exec rm -rf {} + 2>/dev/null || true
    
    # Remove pip cache
    echo "🧹 Cleaning pip cache..."
    sudo -u $ORIGINAL_USER rm -rf $USER_HOME/.cache/pip
    
    # Clean poetry cache
    echo "🧹 Cleaning poetry cache..."
    sudo -u $ORIGINAL_USER rm -rf $USER_HOME/.cache/pypoetry
    
    # Remove old Jupyter notebooks and checkpoints
    ...
    # Verify system Python is default
    echo "🐍 Verifying Python configuration..."
    if [ -f "/usr/local/bin/python" ]; then
        rm -f /usr/local/bin/python
    fi
    
    # Ensure core development tools are installed
    echo "🛠️ Verifying core development tools..."
    pacman -S --needed --noconfirm base-devel git cmake
    
    # Check disk space
    echo "💾 Checking disk space..."
    df -h / | awk 'NR==2 {print $5+0}' | grep -q '^[0-9]\{1,2\}$' || {
        echo "Warning: Disk usage high. Running additional cleanup..."
...
systemctl is-active docker thermald || true
...
echo "  - clean     : Clean system caches"
echo "  - gauntlet  : Activate Gauntlet environment" 
...
if [ ! -d "${CURSOR_PROJECT}/.git" ]; then
    ...
    git -C "${CURSOR_PROJECT}" init
    git -C "${CURSOR_PROJECT}" add .
    git -C "${CURSOR_PROJECT}" commit -m "Initial commit"
fi
...
while true; do
    ...
    find "${CURSOR_PROJECT}" -type f -name "*.md" -exec cat {} + > "${GAUNTLET_HOME}/context/documentation.txt" 2>/dev/null || true
    find "${CURSOR_PROJECT}" -type f -name "*.json" -exec cat {} + > "${GAUNTLET_HOME}/context/config.txt" 2>/dev/null || true

    # Collect metrics if git repo exists
    if [ -d "${CURSOR_PROJECT}/.git" ]; then
        git -C "${CURSOR_PROJECT}" status --porcelain > "${GAUNTLET_HOME}/metrics/git_status.txt" 2>/dev/null || true
        git -C "${CURSOR_PROJECT}" log --since="1 day ago" --pretty=format:"%h %s" > "${GAUNTLET_HOME}/metrics/recent_commits.txt" 2>/dev/null || true
    fi

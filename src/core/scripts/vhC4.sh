...
monitor_services() {
    ...
    for service in context-crystallizer essence-integrator; do
        ...
        if echo "$status" | grep -q "^ActiveState=active"; then
    ...
    for service in context-crystallizer essence-integrator; do
        ...
        if [[ -s "$PATTERN_DB" ]]; then
            ...
            if [[ -n "$service_patterns" ]] && [[ "$service_patterns" != "null" ]]; then
                ...
                if (( $(echo "$current_pressure > $pattern_pressure" | bc -l 2>/dev/null || echo 0) )); then
...
while true; do
    # Send watchdog notification if available
    if command -v systemd-notify >/dev/null 2>&1; then

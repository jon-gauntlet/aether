...
import { TypeValidationResult, PredictionPattern } from '../types/autonomic';
...
export class PredictiveValidation {
  ...
  private validateDevelopmentFlow(energy: EnergyState) {
    ...
    const developmentPatterns = this.patterns$.value.filter(p =>
  ...
  private patternsAreRelated(p1: PredictionPattern, p2: PredictionPattern): boolean {
    return p1.context.some(c => p2.context.includes(c)) ||
           p1.signature.some(s => p2.signature.includes(s));
  }

...
track_editor_sessions() {
    ...
    local editor_count
    editor_count=$(pgrep -f "cursor|code" | wc -l)
...
track_typing_speed() {
    ...
    if [[ -f "$FLOW_STATE_FILE" ]] && [[ $(cat "$FLOW_STATE_FILE") == "1" ]]; then
        local key_count
        key_count=$(xinput list-props "AT Translated Set 2 keyboard" 2>/dev/null | grep "Key Press Count" | awk '{print $NF}')
        if [[ -n "$key_count" ]]; then
            local last_count
...
track_active_files() {
    ...
    if [[ -f "$FLOW_STATE_FILE" ]] && [[ $(cat "$FLOW_STATE_FILE") == "1" ]]; then
        local active_files
        active_files=$(lsof -p $(pgrep -f "cursor|code") 2>/dev/null | grep REG | grep -E '\.(py|js|ts|go|rs|cpp|h|c|java|sh|md)$' | awk '{print $9}' | sort -u)
...
track_git_activity() {
    ...
    if [[ -d "${HOME}/workspace" ]]; then
        local git_changes
        git_changes=$(cd "${HOME}/workspace" && git status --porcelain 2>/dev/null | wc -l)

...
monitor_health_loop() {
    log_monitor "INFO" "Starting health monitoring loop"
    ...
    while true; do
        ...
        case "$status" in
            ...
            "warning"|"degraded")
                ...
                ;;
            *)
                sleep 300  # Normal check interval
                ;;
        esac
    done
}
...
monitor_resources_loop() {
    log_monitor "INFO" "Starting resource monitoring loop"
...
# Export functions
export -f monitor_health_loop
export -f monitor_resources_loop 
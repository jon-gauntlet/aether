...
mkdir -p "$GAUNTLET_HOME/"{state,logs,metrics,context,optimizations}
mkdir -p "$GAUNTLET_HOME/context/"{system,projects,cursor,adhd}
mkdir -p "$GAUNTLET_HOME/optimizations/"{pending,applied,templates}
...
while true; do \
    ...
    if [ -d "$GAUNTLET_HOME/optimizations/pending" ] && [ "$(ls -A "$GAUNTLET_HOME/optimizations/pending" 2>/dev/null)" ]; then \
        for opt in "$GAUNTLET_HOME/optimizations/pending"/*; do \
            if [ -x "$opt" ]; then \
                "$opt" && mv "$opt" "$GAUNTLET_HOME/optimizations/applied/"; \
            fi \
        done \
    fi; \
    \

...
class RiskScorer:
    ...
    def calculate_risk_score(self, finding: Dict[str, Any]) -> Dict[str, Any]:
        """Calculate OWASP risk score based on likelihood and impact."""
        ...
        # Adjust scores based on Rails security category
        category = finding.get('security_category', '')
        if category.startswith('1. Sessions'):
            # Session security is critical for Rails apps
            impact *= 1.5
        elif category.startswith('5. User Management'):
            # User management affects core functionality
            impact *= 1.3
        elif category.startswith('7. XSS'):
            # XSS is a common attack vector
            likelihood *= 1.4
        elif category.startswith('11. Dependency'):
            # Dependency issues often have known exploits
            likelihood *= 1.2
            
        # Adjust based on finding type
        finding_type = finding.get('finding_type', '')
        if finding_type == 'vulnerable_code':
            likelihood *= 1.3  # Code vulnerabilities are more likely to be exploited
        elif finding_type == 'configuration':
            impact *= 1.2  # Config issues often affect entire application
        elif finding_type == 'dependency':
            likelihood *= 1.1  # Dependencies often have public exploits

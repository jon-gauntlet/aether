    ...
    def process_findings(self, input_file: str, output_file: str):
        ...
        for finding in findings:
            ...
            if finding.get('source', {}).get('type') in ['sast', 'dast']:
                ...
                # Normalize finding_type to match schema
                current_type = finding.get('finding_type', '')
                if current_type in ['insecure_defaults', 'misconfiguration']:
                    finding['finding_type'] = 'configuration'
                elif current_type == 'information_disclosure':
                    finding['finding_type'] = 'design'
                elif 'dependency' in current_type.lower() or 'cve' in current_type.lower():
                    finding['finding_type'] = 'dependency'
                elif current_type not in ['vulnerable_code', 'configuration', 'dependency', 'design']:
                    finding['finding_type'] = 'configuration'  # Default for security header/config issues

...
class BearerParser(BaseParser):
    ...
    def __init__(self, config: Dict[str, Any], schema: Dict[str, Any]):
        ...
        """
        # Remove severity from required fields since it's determined by list
        if 'severity' in config.get('required_fields', []):
            config['required_fields'].remove('severity')
    ...
    def parse(self, content: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for severity_level in ['low', 'medium', 'high', 'critical', 'warning']:
            findings = content.get(severity_level, [])
            if not findings:
                continue
    ...
    def _determine_finding_type(self, finding: Dict[str, Any]) -> str:
        ...
        elif any(x in finding_id for x in ['leak', 'exposure', 'disclosure', 'logger']):
    ...
    def _map_category(self, finding: Dict[str, Any]) -> str:
        ...
        elif 'encryption' in finding_id:
            return '3. Data Protection'
    ...
    def _determine_severity(self, severity_level: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'score': 5.0,  # Default score since Bearer doesn't provide one
            'justification': 'Severity level determined by Bearer categorization'
        }
    ...
    def _generate_tags(self, finding: Dict[str, Any]) -> List[str]:
        ...
        # Add finding type
        finding_type = self._determine_finding_type(finding)
        tags.append(f"type:{finding_type}")

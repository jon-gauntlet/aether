...
generate_ai_context() {
    ...
    # Generate JSON version with error handling
    if ! jq -n \
        ...
        echo "Error generating AI context JSON, using fallback format"
        echo "{}" > "$context_file"
        return 1
    fi
    ...
    # Generate markdown version
    {
        ...
        if [[ -f "$metrics_file" ]]; then
            jq -r '. | to_entries | .[] | "- \(.key): \(.value)"' "$metrics_file" 2>/dev/null || echo "No metrics available"
        else
        ...
        if [[ -f "$learning_file" ]]; then
            cat "$learning_file" 2>/dev/null || echo "No learning model available"
        else
        ...
        if [[ -f "$advanced_file" ]]; then
            jq -r '. | to_entries | .[] | "### \(.key)\n\(.value | to_entries | map("- \(.key): \(.value)") | .[])"' "$advanced_file" 2>/dev/null || echo "No advanced context available"
        else
            echo "No advanced context available"
        fi
    } > "${context_file}.md"
    
    # Update Cursor context with error handling
    if [[ -f "${CURSOR_CONFIG_DIR}/context.json" ]]; then
        local cursor_context
        cursor_context=$(cat "${CURSOR_CONFIG_DIR}/context.json")
        
        # Add AI context with error handling
        if ! echo "$cursor_context" | jq --arg path "$project_path" --arg cf "$context_file" \
            '. + {ai_context: {($path): $cf}}' > "${CURSOR_CONFIG_DIR}/context.json.tmp" 2>/dev/null; then
            echo "Error updating Cursor context, skipping update"
            return 1
        fi
        mv "${CURSOR_CONFIG_DIR}/context.json.tmp" "${CURSOR_CONFIG_DIR}/context.json"
    fi
}
...
track_ai_feedback() {
    ...
    if [[ -f "$context_file" ]] && [[ -f "$strategy_file" ]]; then
        ...
        jq -n \
            ...
            --argjson fi "${focus_improvement:-0}" \
            --argjson qi "${quality_improvement:-0}" \
            --argjson di "${doc_improvement:-0}" \
            '{
                project_path: $path,
                timestamp: $ts,
                metrics: {
                ...
                effectiveness_score: ((($fi // 0) / 3600 + ($qi // 0) + ($di // 0)) / 3)
            }' > "$feedback_file" 2>/dev/null || echo "{}" > "$feedback_file"
    fi
}

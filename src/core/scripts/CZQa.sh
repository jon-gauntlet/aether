...
ExecStart=/bin/bash -c 'while true; do \
    ...
    inotifywait -r -e modify,create,delete $CHATGENIUS_DIR > $CHATGENIUS_DIR/.gauntlet/file_changes.log 2>/dev/null || true; \
    \
    # Update project context \
    find $CHATGENIUS_DIR -type f -name "*.md" -exec cat {} + > $CHATGENIUS_DIR/.gauntlet/documentation.txt 2>/dev/null || true; \
    find $CHATGENIUS_DIR -type f -name "*.json" -exec cat {} + > $CHATGENIUS_DIR/.gauntlet/config.txt 2>/dev/null || true; \
    \
    # Collect metrics \
    git -C $CHATGENIUS_DIR status --porcelain > $CHATGENIUS_DIR/.gauntlet/git_status.txt 2>/dev/null || true; \
    git -C $CHATGENIUS_DIR log --since="1 day ago" --pretty=format:"%h %s" > $CHATGENIUS_DIR/.gauntlet/recent_commits.txt 2>/dev/null || true; \
    \
    sleep 60; \
done'

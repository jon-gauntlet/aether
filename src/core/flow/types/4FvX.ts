...
export class NaturalFlow {
  ...
  private evolveThoughtStreams() {
    ...
    for (let i = 0; i < streams.length; i++) {
      for (let j = i + 1; j < streams.length; j++) {
        const streamA = streams[i] as ThoughtStream;
        const streamB = streams[j] as ThoughtStream;
  ...
  private adjustTemperature() {
    ...
    const avgTemp = Array.from(currentState.streams.values())
      .reduce((sum: number, s: ThoughtStream) => sum + s.temperature, 0) / 
      currentState.streams.size || 0;
  ...
  private emergePatterns() {
    ...
    streams
      .filter((s: ThoughtStream) => s.temperature > 0.7)
      .forEach((stream: ThoughtStream) => {
        const connected = stream.connections
          .map(id => currentState.streams.get(id))
          .filter((s): s is ThoughtStream => s !== undefined && s.temperature > 0.7);

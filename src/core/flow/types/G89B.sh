...
# Gauntlet Focus Assistant (gf)
# Smart focus tracking optimized for 80-100hr weeks
#
# Quick Start: Just type 'gf' and discover as you go
# Each command shows relevant next steps
...
# Smart help that grows with usage
show_help() {
    # Count user's command usage
    USAGE_FILE="$HOME/.local/share/gauntlet/command_usage"
    touch "$USAGE_FILE"
    
    # Basic commands always shown
    cat << EOF
Gauntlet Focus Assistant
════════════════════════

Start Here:
  gf         Quick focus check (shows next steps)
  
$(grep "flow" "$USAGE_FILE" >/dev/null || echo "Try This Next:
  gf flow    Optimize for hyperfocus
")
EOF

    # Show more commands based on usage
    grep "flow" "$USAGE_FILE" >/dev/null && cat << EOF
Flow Tools:
  gf deep    Zero-distraction mode
  gf zen     Minimal display mode
  
Analysis:
  gf when    Find your peak hours
  gf stats   View focus patterns
EOF

    exit 0
}

# Record command usage for smart help
record_usage() {
    USAGE_FILE="$HOME/.local/share/gauntlet/command_usage"
    mkdir -p "$(dirname "$USAGE_FILE")"
    echo "$(date +%s) $1" >> "$USAGE_FILE"
}

[[ "$1" == "help" || "$1" == "--help" || "$1" == "-h" ]] && show_help

# Handle focus modes (records usage for smart help)
case "$1" in
    quiet|q) QUIET=1; record_usage "quiet" ;;
    mini|minimal) MINIMAL=1; record_usage "minimal" ;;
    zen) NO_COLOR=1; QUIET=1; record_usage "zen" ;;
    flow) FLOW=1; record_usage "flow" ;;
    deep) DEEP=1; record_usage "deep" ;;
    watch) WATCH=1; record_usage "watch" ;;
    stats) STATS=1; record_usage "stats" ;;
    when) WHEN=1; record_usage "when" ;;
    *) record_usage "basic" ;;
esac
...
show_next_steps() {
    [[ -f "$FLOW_STATE" ]] && {
        echo -e "\n${BLUE}Flow State Active:${NC}"
        echo "- Use 'gf deep' to eliminate distractions"
        echo "- Check 'gf stats' to track your progress"
        return
    }
    
    echo -e "\n${BLUE}Next Steps:${NC}"
    grep "flow" "$USAGE_FILE" >/dev/null || echo "- Try 'gf flow' to optimize your environment"
    grep "when" "$USAGE_FILE" >/dev/null || echo "- Use 'gf when' to find your peak hours"
    grep "stats" "$USAGE_FILE" >/dev/null || echo "- Check 'gf stats' to track progress"
}

...
track_cursor_session() {
    ...
    if [[ -f "${METRICS_DIR}/active_files" ]]; then
        active_files=$(jq -R -s -c 'split("\n")[:-1]' "${METRICS_DIR}/active_files" 2>/dev/null || echo "[]")
    else
        active_files="[]"
    fi
    ...
    if [[ -d "$project_path" ]]; then
        ...
        local commit
        commit=$(git log -1 --pretty=format:%h 2>/dev/null || echo "unknown")
        local changes
        ...
        git_context=$(jq -n \
            ...
            --arg commit "$commit" \
            --arg changes "$changes" \
            '{branch: $branch, last_commit: $commit, changes: ($changes|tonumber)}')
    fi

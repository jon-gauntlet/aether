...
describe('FlowEngine', () => {
  ...
  describe('initialization', () => {
    it('should initialize with default values', () => {
      const flow = engine.observe().pipe(take(1)).toPromise();
      expect(flow).resolves.toMatchObject({
        state: 'natural',
        metrics: {
          focus: 1,
          presence: 1,
  ...
  describe('deepening', () => {
    it('should increase depth on deepen()', async () => {
      ...
      expect(updatedFlow.metrics.focus).toBeGreaterThan(initialFlow.metrics.focus);
    });
  });
}); 
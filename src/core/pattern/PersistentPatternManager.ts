...
export class PersistentPatternManager extends PatternManager {
    ...
    try {
      ...
      if (stored) {
        ...
        data.patterns.forEach((pattern: Pattern) => {
          ...
          pattern.meta.learnings.forEach(learning => {
  ...
  getPatternStats() {
    return {
      ...
      totalLearnings: this.patterns.reduce((sum, pattern) => 
        sum + pattern.meta.learnings.length, 0),
      averageSuccessRate: this.patterns.reduce((sum, pattern) => 
        sum + pattern.meta.successRate, 0) / (this.patterns.length || 1),
      mostUsedPattern: this.patterns.reduce((most, current) => 
        current.meta.useCount > (most?.meta.useCount || 0) ? current : most, null as Pattern | null),

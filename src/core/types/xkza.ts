...
export class Integrate {
  ...
  private settleSystem: Settle;
  
  constructor(moveSystem: Move) {
    ...
    this.settleSystem = new Settle(moveSystem);
    this.watch();
  }

  private watch() {
    // Let understanding flow naturally
    this.moveSystem.watch().subscribe(flow => {
      const wave = this.wave.value;
      
      // Depth grows in quiet
      this.settleSystem.see().subscribe(rest => {
        if (rest.quiet > 0.8) {
  ...
  spread() {
    ...
    this.moveSystem.watch().subscribe(flow => {
      ...
      flow.spots.forEach((spot: Spot) => {
        spot.feel.depth = blend(spot.feel.depth, wave.depth);
      });
      
      // And spreads through ways
      flow.ways.forEach((way: Way) => {
        way.feel.depth = blend(way.feel.depth, wave.depth * wave.spread);
      });
      
      // And settles in groups
      flow.groups.forEach((group: Group) => {

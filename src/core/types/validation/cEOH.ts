...
const validateWave = (wave: unknown): boolean => {
  if (!validateObject(wave)) return false;
  const { frequency, amplitude, phase } = wave as Wave;
  return (
    validateNumber(frequency) &&
    validateNumber(amplitude) &&
    validateNumber(phase)
  );
};

export const validateField = (field: Field): boolean => {
  if (!validateObject(field)) return false;
  const { center, radius, strength, waves } = field;
  ...
  if (!isValidMeasure(strength)) return false;
  if (!validateArray(waves, validateWave)) return false;

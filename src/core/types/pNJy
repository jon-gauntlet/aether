...
optimize_context() {
    ...
    def score(obj):
        ...
        (obj.frequency // 0) * 1.5 +
        (obj.recent // 0) * 3 +
        (if obj.type == "sacred" then 10 else 0 end);
    ...
    
    {
        ...
        state: (.state | with_entries(
            select(.value.priority > 0.7 or .value.type == "sacred")
        )),
...
watch_context_changes() {
    ...
    while true; do
        ...
        if [[ -f "$ACTIVE_CONTEXT" ]]; then
            ...
            if (( current_size > last_size * 1.5 || current_size > MAX_CONTEXT_SIZE * OPTIMIZATION_THRESHOLD )); then

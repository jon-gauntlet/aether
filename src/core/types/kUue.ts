...
export class Flow implements FlowInterface {
  ...
  observeDepth(): Observable<number> {
    return this.observeFlow().pipe(map(state => state.depth));
  }

  observeEnergy(): Observable<number> {
    return this.observeFlow().pipe(map(state => state.energy));
  }

  observeFocus(): Observable<number> {
    return this.observeFlow().pipe(map(state => (state.depth + state.energy) / 2));
  }
}
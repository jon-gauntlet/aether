...
export class Flow {
  ...
  private tend() {
    ...
    this.streams.forEach(stream => {
      ...
      // Update flow state
      stream.flowState = this.determineFlowState(stream);
    });
  }
  ...
  private getPresenceTypeAffinity(type1: PresenceType, type2: PresenceType): number {
    ...
    const affinities: Record<PresenceType, Record<PresenceType, number>> = {
      reading: { writing: 0.3, thinking: 0.5, listening: 0.2 },
      writing: { reading: 0.3, thinking: 0.4, listening: 0.2 },
      thinking: { reading: 0.5, writing: 0.4, listening: 0.3 },
      listening: { reading: 0.2, writing: 0.2, thinking: 0.3 }
    } as const;
  ...
  private determineFlowState(stream: Stream): FlowState {
    const { presence, stillness, depth, timeInState = 0 } = stream;

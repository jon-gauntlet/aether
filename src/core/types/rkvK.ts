...
export class PredictiveValidation {
  ...
  public async validateState(state: ConsciousnessState): Promise<TypeValidationResult> {
    ...
    if (state.metrics) {
      const metricsValidation = await this.validateSystemMetrics(state.metrics, ['metrics']);
  ...
  private async estimateSuccessProbability(
    ...
    try {
      const systemState = await autonomic.observeSystemState().pipe(take(1)).toPromise();
      return Math.min(
        1,
        (pattern.strength * energy.current * systemState.metrics.coherence) / 3
      );
    } catch (error) {

...
export class FlowStateGuardian {
  ...
  private evaluateHyperfocus(context: FlowContext, current: HyperfocusMetrics): HyperfocusMetrics {
    ...
    return {
      ...
      depth: context.depth,
      clarity: current.clarity * (isHyperfocus ? 1.1 : 0.95),
      focus: newIntensity * current.contextRetention,
      energy: current.energy * (isHyperfocus ? 0.98 : 1.02),
      resonance: current.resonance * (isHyperfocus ? 1.05 : 0.95),
      presence: current.presence * (isHyperfocus ? 1.1 : 0.9),
      harmony: current.harmony * (isHyperfocus ? 1.05 : 0.95),
      rhythm: current.rhythm * (isHyperfocus ? 1.1 : 0.9)
    };
  }

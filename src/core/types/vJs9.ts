...
export class AutonomicSystem {
  ...
  private calculateEnergyAlignment(energy: any): number {
    // Natural alignment calculation
    const stateAlignment = energy.resonance * 0.4;    // Natural state harmony
    const contextAlignment = energy.protection * 0.3;  // Context preservation
    const flowAlignment = (energy.type === 'flow' ? 1 : 0.5) * 0.3; // Flow recognition
  ...
  private calculatePatternResonance(energy: any, alignment: number): number {
    ...
    // Natural resonance through harmony
    return Math.min(
      (energy.resonance * 0.4) +     // Current harmony
      (alignment * 0.3) +            // System alignment
      (current.pattern_resonance * 0.3), // Pattern continuity
      1
    );
  }

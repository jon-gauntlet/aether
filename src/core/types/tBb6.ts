...
export class Flow {
  ...
  private tend(delta: number) {
    ...
    this.streams.forEach(subject => {
      ...
      const presenceDecay = this.getPresenceDecay(stream.presenceType, timeSinceActivity);
  ...
  private getPresenceDecay(type?: PresenceType, timeSinceActivity: number): number {

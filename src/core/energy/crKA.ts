...
import { Pattern, StoredPattern, PatternAnalytics } from '../types/patterns';
...
export class EnergySystem {
  ...
  // Pattern Analytics
  getPatternAnalytics(): Observable<{
    patterns: {
      id: string;
      name: string;
    ...
    return this.state$.pipe(
      map(state => {
        const patterns = state.activePatterns.map(pattern => {
          const analytics = this.calculatePatternAnalytics(pattern);
          return {
            id: pattern.id,
            name: pattern.name,
            analytics
        ...
        const systemMetrics = this.calculateSystemMetrics(patterns);
        const insights = this.generateInsights(patterns, systemMetrics);
  ...
  private calculatePatternAnalytics(pattern: Pattern): PatternAnalytics {
    const entropyFactor = this.calculateEntropyFactor(pattern);
    const evolutionMetrics = this.calculateEvolutionMetrics(pattern);
    
    const strengthGrowth = pattern.evolution.history.length > 0
      ? pattern.strength / pattern.evolution.history.length
      : 0;

    const stabilityScore = (1 - entropyFactor) * evolutionMetrics.protectionRate;

    return {
      entropyFactor,
      evolutionRate: evolutionMetrics.growthRate,
      strengthGrowth,
      stabilityScore,
      metrics: evolutionMetrics
    };
  ...
  private calculateSystemMetrics(patterns: { analytics: PatternAnalytics }[]) {
    ...
    const totalPatterns = this.state$.value.activePatterns.length;
    const activePatterns = patterns.length;
    
    const averageStrength = this.state$.value.activePatterns.reduce(
      (sum, p) => sum + p.strength, 0
    ) / totalPatterns;

    const averageEntropy = patterns.reduce(
      (sum, p) => sum + p.analytics.entropyFactor, 0
    ) / activePatterns;

    const evolutionRate = patterns.reduce(
      (sum, p) => sum + p.analytics.evolutionRate, 0
    ) / activePatterns;

    // Calculate pattern synergy based on co-occurrence in sessions
    const patternSynergy = this.calculatePatternSynergy();
  ...
  private calculatePatternSynergy(): number {
    const patterns = this.state$.value.activePatterns;
    if (patterns.length < 2) return 0;

    const coOccurrenceMatrix: Record<string, Record<string, number>> = {};
    patterns.forEach(p1 => {
      coOccurrenceMatrix[p1.id] = {};
      patterns.forEach(p2 => {
        if (p1.id !== p2.id) {
          coOccurrenceMatrix[p1.id][p2.id] = 0;
        }
      });
    });

    // Calculate co-occurrences in sessions
    this.state$.value.flowHistory.forEach(session => {
      session.patterns.forEach(p1 => {
        session.patterns.forEach(p2 => {
          if (p1 !== p2 && coOccurrenceMatrix[p1]?.[p2] !== undefined) {
            coOccurrenceMatrix[p1][p2]++;
          }
        });
      });
    });
    ...
    patterns.forEach(p1 => {
      patterns.forEach(p2 => {
        if (p1.id !== p2.id) {
          totalCoOccurrences += coOccurrenceMatrix[p1.id][p2.id];
          pairCount++;
        }
      });
    });

    return pairCount > 0 ? totalCoOccurrences / pairCount : 0;
  }

  private generateInsights(
    patterns: { id: string; name: string; analytics: PatternAnalytics }[],
    systemMetrics: { averageEntropy: number; evolutionRate: number }
  ) {
    const insights: {
      type: 'success' | 'warning' | 'info';
      message: string;
    ...
    // Check for patterns ready to evolve
    const readyPatterns = patterns.filter(
      p => p.analytics.metrics.readyToEvolve
    );
    if (readyPatterns.length > 0) {
      insights.push({
        type: 'success',
        message: `${readyPatterns.length} pattern(s) ready to evolve`,
        patterns: readyPatterns.map(p => p.id)
      });
    }

    // Check for high entropy patterns
    const highEntropyPatterns = patterns.filter(
      p => p.analytics.entropyFactor > 0.7
    );
    if (highEntropyPatterns.length > 0) {
      insights.push({
        type: 'warning',
        message: `${highEntropyPatterns.length} pattern(s) need recovery`,
        patterns: highEntropyPatterns.map(p => p.id)
      });
    }

    // System-level insights
    if (systemMetrics.averageEntropy > 0.5) {
      insights.push({
        type: 'warning',
        message: 'System entropy is high, consider rest state'
      });
    }

    if (systemMetrics.evolutionRate < 0.3) {
      insights.push({
        type: 'info',
        message: 'Pattern evolution rate is low, try increasing intensity'
      });
    }

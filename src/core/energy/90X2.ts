...
describe('useAutonomicDevelopment', () => {
  ...
  it('cleans up subscriptions on unmount', () => {
    ...
    const mockSubscribe = vi.fn().mockReturnValue({ unsubscribe: unsubscribeSpy });

    const mockFlow$ = new Observable<AutonomicFlow>(subscriber => {
      const subscription = mockSubscribe();
      subscriber.next(flow$.getValue());
      return subscription;
    });

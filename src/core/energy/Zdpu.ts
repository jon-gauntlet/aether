...
import { Flow, FlowEngine, PresenceType } from '../experience/flow';

const flowEngine = new FlowEngine();

export function useFlow(): Flow {
  const [stream, setStream] = useState<Flow['stream']>(undefined);
  const [otherStreams, setOtherStreams] = useState<Flow['otherStreams']>([]);
  ...
  useEffect(() => {
    ...
    const subscription = flowEngine.observe(id).subscribe(setStream);

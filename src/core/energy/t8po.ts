...
describe('Autonomic Validation System', () => {
  ...
  it('should evolve autonomic behavior over time', async () => {
    ...
    await act(async () => {
      ...
      expect(state?.proactivity ?? 0).toBeLessThan(0.7);
      expect(state?.confidence ?? 0).toBeLessThan(0.8);
    });
    ...
    await act(async () => {
      const state = await result.current.getAutonomicState().toPromise();
      expect(state?.proactivity ?? 0).toBeGreaterThan(0.7);
      expect(state?.confidence ?? 0).toBeGreaterThan(0.8);
    });
  });

...
export class PredictiveValidation {
  ...
  public async recordValidation(
    context: string[],
    success: boolean,
    options?: { timing?: number }
  ): Promise<void> {
    const patterns = this.patterns$.value;
    let pattern = patterns.find(p => p.context.join(',') === context.join(','));

    if (!pattern) {
      pattern = this.createPattern(context);
      patterns.push(pattern);
    }

    // Update validation history
    pattern.indicators.validation_success.push(success ? 1 : 0);
    if (pattern.indicators.validation_success.length > 10) {
      pattern.indicators.validation_success.shift();
    }

    // Update timing if provided
    if (options?.timing) {
      pattern.predictions.timing = options.timing;
      pattern.predictions.optimal_timing = options.timing;
    }

    // Update pattern strength based on validation
    const successRate = pattern.indicators.validation_success.reduce((a, b) => a + b, 0) / 
                       pattern.indicators.validation_success.length;
    pattern.strength = (pattern.strength * 0.7) + (successRate * 0.3);

    // Record autonomic decision
    const energy = await this.energySystem.observeEnergy().pipe(take(1)).toPromise();
    if (!energy) throw new Error('Failed to get energy state');

    const autonomicState = await this.autonomic.observeAutonomicState().pipe(take(1)).toPromise();
    if (!autonomicState) throw new Error('Failed to get autonomic state');

    this.autonomic.recordDecision({
      success_rate: success ? 1 : 0,
      resonance: pattern.strength,
      depth: energy.level,
      protection_level: autonomicState.flow_protection
    });

...
export class SystemOperations {
  ...
  private calculateHarmonizedFlow(
    ...
    return {
      ...
      coherence: Math.min((source.coherence + target.coherence) / 2 + 0.1, 1),
      presence: Math.min((source.presence + target.presence) / 2 + 0.1, 1),
      harmony: Math.min((source.harmony + target.harmony) / 2 + 0.1, 1)
    };
  }

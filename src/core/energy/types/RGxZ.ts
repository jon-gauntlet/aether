...
export class EnergyEngine {
  ...
  // Add index tracking for core system components
  private patternIndex = new Map<string, {
    type: 'energy' | 'flow' | 'autonomic' | 'consciousness',
    patterns: Set<string>
  }>();
  
  private systemIndex = {
    energyPatterns: new Set<string>(),
    flowPatterns: new Set<string>(),
    autonomicPatterns: new Set<string>(),
    consciousnessPatterns: new Set<string>()
  };
  ...
  // Update pattern creation with indexing
  ...
  private indexPattern(pattern: EnergyPattern): void {
    // Index by pattern type
    const type = this.determinePatternType(pattern);
    if (!this.patternIndex.has(type)) {
      this.patternIndex.set(type, {
        type,
        patterns: new Set()
      });
    }
    this.patternIndex.get(type)?.patterns.add(pattern.id);

    // Index in system index
    switch(type) {
      case 'energy':
        this.systemIndex.energyPatterns.add(pattern.id);
        break;
      case 'flow':
        this.systemIndex.flowPatterns.add(pattern.id);
        break;
      case 'autonomic':
        this.systemIndex.autonomicPatterns.add(pattern.id);
        break;
      case 'consciousness':
        this.systemIndex.consciousnessPatterns.add(pattern.id);
        break;
    }
  }

  private determinePatternType(pattern: EnergyPattern): 'energy' | 'flow' | 'autonomic' | 'consciousness' {
    if (pattern.quality === 'vital' || pattern.quality === 'purifying') return 'energy';
    if (pattern.quality === 'flowing' || pattern.quality === 'transformative') return 'flow';
    if (pattern.quality === 'protective') return 'autonomic';

...
import { ProtectionGuardian } from './ProtectionGuardian';
import type { ProtectionState } from './ProtectionGuardian';
...
export class TypeGuardian {
  ...
  private protectionGuardian: ProtectionGuardian;

  constructor(private rootDir: string) {
    this.initializeWatcher();
    const program = this.fileWatcher.getProgram()?.getProgram();
    if (!program) throw new Error('Failed to initialize program');
    this.protectionGuardian = new ProtectionGuardian(program);
    this.startContinuousValidation();
    this.initializeAutonomicRules();
    this.startEnergyManagement();
    this.initializeProtectionSync();
  }

  private initializeProtectionSync() {
    // Sync energy and mode with protection guardian
    this.protectionGuardian.observeProtection().subscribe(protectionState => {
      this.state$.next({
        ...this.state$.value,
        energy: protectionState.energy,
        mode: protectionState.mode
      });
    });

    // Sync invariants with type system
    setInterval(() => {
      const invariants = this.protectionGuardian.getInvariants();
      const typeMap = new Map<string, Set<string>>();

      // Convert invariants to type map
      invariants.types.forEach(type => {
        typeMap.set(type, new Set());
      });

      invariants.relationships.forEach(rel => {
        const [derived, base] = rel.split(' extends ');
        if (typeMap.has(derived)) {
          typeMap.get(derived)!.add(base);
        }
      });
  ...
  public observeProtection(): Observable<ProtectionState> {
    return this.protectionGuardian.observeProtection();
  }
} 
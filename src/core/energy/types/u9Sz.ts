...
export interface IndexStats {
  lastIndexed: Date;
  indexedCount: number;
  patternMetrics: Map<string, PatternIndex>;
}

export const hasPattern = (pattern: any): pattern is Pattern => {
  return pattern && typeof pattern === 'object' && 'id' in pattern;
};

export const isEnergyOptimized = (metrics: EnergyFlowMetrics): boolean => {
  return metrics.efficiency > 0.7 && metrics.coherence > 0.7;
};

export interface Space {
  id: string;
  type: 'development' | 'reflection' | 'integration';
  metrics: {
    energy: number;
    focus: number;
    progress: number;

...
export class FlowEngine implements Flow {
  ...
  setMode(mode: NaturalFlowType): void {
    ...
    // Ensure we emit both the old and new state
    if (oldType !== mode) {
      this.flowSubject.next({ ...this, type: oldType, timestamp: now - 1 });
      this.flowSubject.next({ ...this, type: mode, timestamp: now });
    } else {
  ...
  notice(id: string, type: string): void {
    ...
    if (stream) {
      ...
      this.timestamp = now + 1; // Ensure engine timestamp is after stream timestamp
  ...
  private updateState(changes: Partial<NaturalFlow>): void {
    ...
    this.timestamp = now + 1; // Ensure timestamp is updated
    this.flowSubject.next(this);
  }
}
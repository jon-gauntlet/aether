...
export class FlowEngine {
  ...
  public observe(): Observable<FlowPattern[]> {
    return this.flows.pipe(
      map(flowIds => Array.from(flowIds)
        .map(id => this.patterns.get(id)?.value)
        .filter((pattern): pattern is FlowPattern => pattern !== undefined)
      )
    );
  }
  ...
  private async updateResonance(): Promise<void> {
    ...
    const resonance: Resonance = {
      ...
      harmony: this.calculateHarmony(patterns),
      essence: patterns.reduce((acc, p) => acc + p.state.resonance.essence, 0) / patterns.length,
      field: this.createInitialField()
    };
  ...
  public async adjust(id: string, strength: number): Promise<void> {
    const pattern = this.patterns.get(id);
    if (!pattern) return;
    ...
    const updated: FlowPattern = {
      ...
      state: {
        ...current.state,
        energy: Math.min(1, Math.max(0, strength)),
        timestamp: Date.now()
      }
    };

    pattern.next(updated);

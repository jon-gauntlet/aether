...
show_focus_metrics() {
    ...
    if [[ -f "${METRICS_DIR}/context_switch.log" ]]; then
        context_switches=$(grep -c "^$(date +%Y%m%d_%H)" "${METRICS_DIR}/context_switch.log" 2>/dev/null | tr -d '\n' || echo "0")
    fi
...
show_productivity_metrics() {
    ...
    if [[ -f "${METRICS_DIR}/typing_speed.log" ]]; then
        typing_speed=$(tail -n 1 "${METRICS_DIR}/typing_speed.log" 2>/dev/null | awk '{print $2}' | tr -d '\n' || echo "0")
    fi
    
    # Get git changes
    local git_changes="0"
    if [[ -f "${METRICS_DIR}/git_changes.log" ]]; then
        git_changes=$(tail -n 1 "${METRICS_DIR}/git_changes.log" 2>/dev/null | awk '{print $2}' | tr -d '\n' || echo "0")
    fi
    
    # Get active editor sessions
    local editor_sessions="0"
    if [[ -f "${METRICS_DIR}/editor_sessions.log" ]]; then
        editor_sessions=$(tail -n 1 "${METRICS_DIR}/editor_sessions.log" 2>/dev/null | awk '{print $2}' | tr -d '\n' || echo "0")
    fi
...
show_system_metrics() {
    ...
    if [[ -f "${METRICS_DIR}/system_load.log" ]]; then
        system_load=$(tail -n 1 "${METRICS_DIR}/system_load.log" 2>/dev/null | cut -d' ' -f2- | tr -d '\n' || echo "N/A")
    fi
    
    # Get latest memory usage
    local memory_usage="0"
    if [[ -f "${METRICS_DIR}/memory_usage.log" ]]; then
        memory_usage=$(tail -n 1 "${METRICS_DIR}/memory_usage.log" 2>/dev/null | awk '{print $2}' | tr -d '\n' || echo "0")
    fi
...
show_recommendations() {
    ...
    if [[ -f "${STATE_DIR}/flow_state" ]] && [[ $(cat "${STATE_DIR}/flow_state" 2>/dev/null | tr -d '\n') == "1" ]]; then

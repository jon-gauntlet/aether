...
def load_findings(config, findings):
    for category in ['dast', 'sast', 'code-llm']:
        ...
        logging.info(f"Loaded {len(findings[category])} findings for category {category}. Sample: {findings[category][:1] if len(findings[category]) > 0 else 'No data'}")
...
def main():
    ...
    try:
        ...
        extracted_path = os.path.join(config['paths']['output_dir'], 'extracted-findings.json')
        write_json(findings, extracted_path)

        transformed_findings = transform_findings(findings)
        transformed_path = os.path.join(config['paths']['output_dir'], 'transformed-findings.json')
        write_json(transformed_findings, transformed_path)

        enriched_findings = enrich_findings(transformed_findings)
        enriched_path = os.path.join(config['paths']['output_dir'], 'enriched-findings.json')
        ...
        logging.error("Error in ETL process: %s", str(e))

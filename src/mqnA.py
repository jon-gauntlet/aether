...
def setup_logging():
    ...
    log_dir = '/home/jon/.local/share/cursor/logs'
    try:
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(os.path.join(log_dir, 'context_service.log')),
                logging.StreamHandler(sys.stdout)
            ]
        )
    except Exception as e:
        # Fall back to systemd journal if file logging fails
        logging.basicConfig(
            level=logging.INFO,
            format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
            handlers=[
...
class ContextService:
    ...
    def _ensure_directories(self) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to create/verify directories: {e}")
            raise
    ...
    async def start(self):
        ...
        self._tasks.append(asyncio.create_task(self._pattern_update_task()))
    ...
    async def _update_global_patterns(self):
        """Update global patterns from all contexts."""
        active_dir = os.path.join(self.base_path, 'active')
        patterns = []
        ...
        for filename in os.listdir(active_dir):
            if filename.endswith('.json'):
                try:
                    with open(os.path.join(active_dir, filename)) as f:
                        context = json.load(f)
                    patterns.extend(await self._extract_patterns(context))
                except Exception as e:
    ...
    async def _archive_old_contexts(self):
        """Archive contexts older than retention period."""
        active_dir = os.path.join(self.base_path, 'active')
        archive_dir = os.path.join(self.base_path, 'archive')
        ...
        for filename in os.listdir(active_dir):
            ...
            try:
                ...
                if age > self.boundaries.retention_days:
                    archive_path = os.path.join(archive_dir, filename)
                    os.rename(filepath, archive_path)
                    logger.info(f"Archived old context: {filename}")
            except Exception as e:
                logger.error(f"Error processing {filename}: {e}")

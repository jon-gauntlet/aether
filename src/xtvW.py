...
def test_for_loop_basics():
    ...
    for num in numbers:
        sum_result += num
    assert_equal(__, sum_result, "For loops can sum numbers")
...
def test_while_loop():
    ...
    while counter < 5:
        result += counter
        counter += 1
    assert_equal(__, result, "While loops continue until condition is false")

def test_range_function():
    """Understanding range"""
    numbers = list(range(5))
    print("numbers = list(range(5)). numbers=f{numbers}")
    assert_equal(__, len(numbers), "Range creates a sequence of numbers")
    assert_equal(__, numbers[0], "Range starts at zero by default")
    assert_equal(__, numbers[-1], "Last number is one less than the stop value")
...
def test_loop_control():
    ...
    for i in range(10):
        ...
        if i == 7:
            break    # Stop at 7
        result.append(i)
    assert_equal(__, result, "Break and continue control loop flow")

def test_list_comprehension():
    """Understanding list comprehension"""
    numbers = [1, 2, 3, 4, 5]
    squares = [n * n for n in numbers]
    assert_equal(__, squares, "List comprehension creates new lists")

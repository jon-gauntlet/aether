...
describe('Navigator', () => {
  ...
  describe('observation', () => {
    it('emits navigation updates', (done) => {
      navigator.observeNavigation().pipe(
        take(2),
        toArray()
      ).subscribe((states: NavigationState[]) => {
    ...
    it('includes timestamps in updates', () => {
      const initialTime = navigator.getCurrentState().timestamp;
      navigator.navigate('/test');
      expect(navigator.getCurrentState().timestamp).toBeGreaterThan(initialTime);
    });
  });
  ...
  describe('back navigation', () => {
    ...
    it('maintains params during back navigation', () => {
      navigator.navigate('/first', { id: '123' });
      navigator.navigate('/second', { name: 'test' });
      navigator.goBack();

      // Should restore first route's params
      expect(navigator.getCurrentState().params).toEqual({ id: '123' });
    });
  });
